...
export default function PostPage({ frontMatter, mdxSource, posts }) {
  return (
    <Layout posts={posts} showFontControls={true}>
      <article className="max-w-none mx-auto">
        <header className="mb-12 text-center flex flex-col items-center justify-center">
          <h1 className="text-4xl font-bold mb-6 text-blue-500 dark:text-blue-400 text-center">{frontMatter.title}</h1>
          {frontMatter.excerpt && (
            <p className="text-lg text-gray-600 dark:text-gray-400 leading-relaxed max-w-3xl mx-auto text-center">{frontMatter.excerpt}</p>
          )}
